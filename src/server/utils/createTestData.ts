...
import type { NewPlayer, NewSeason, Season } from "@common/types";
import { DEFAULT_ELO } from "@common/utils/constants";
import { calculateNewGameEffects } from "@server/db/games";
import {
  clearGamesDEV,
  getSeasonGameCountForPlayer,
} from "@server/db/games/derivatives";
import { clearPlayersDEV, getPlayers } from "@server/db/players";
...
const createGames = async () => {
  ...
  const allPlayerData: { model: Player; gameCount: number }[] = allPlayers.map(
    (p) => {
      return { model: p, gameCount: 0 };
    },
  );
  const seasonPlayerData: {
    [playerId: number]: { [seasonId: number]: number };
  } = Object.fromEntries(
    allPlayers.map((p) => [
      p,
      Object.fromEntries(
        seasons.map((s) => [s.id, getSeasonGameCountForPlayer(p.id, s.id)]),
      ),
    ]),
  );
  ...
  games.forEach((game) => {
    ...
    const season = seasons.find(
      (s) => s.start <= createdAt && createdAt < s.end,
    );
    const effects = calculateNewGameEffects(
      {
        winnerId: winnerData.model.id,
        loserId: loserData.model.id,
        underTable,
      },
      {
        ...
        winnerSeasonGames: season
          ? seasonPlayerData[winnerData.model.id]?.[season.id]
          : null,
        loserGames: loserData.gameCount,
        loserSeasonGames: season
    ...
    if (winnerUpdateInfo.seasonElo)
      winnerData.model.seasonElo = winnerUpdateInfo.seasonElo;
    if (winnerUpdateInfo.latestSeasonId)
      winnerData.model.latestSeasonId = winnerUpdateInfo.latestSeasonId;
    if (loserUpdateInfo.seasonElo)
      loserData.model.seasonElo = loserUpdateInfo.seasonElo;
