...
const Testfield: NextPage = () => {
  ...
  const fetchPlayerMetas = async (keywords: string): Promise<ApiState> => {
    ...
    return {
      loading: false,
      ts,
      data: await fetch(`/api/players/search?${new URLSearchParams({ keywords })}`, {
        method: 'GET',
      })
        .then(res => res.json())
        .then(res => {
  ...
  return (
    <div>
      ...
      <input className="border-4 m-8" type="text" value={value} onChange={onChange} />
      {apiState.loading ? (
        <p>Loading...</p>
      ) : apiState.data.length ? (
        apiState.data.map(d => <p key={d.id}>{`${d.firstName} ${d.lastName} (${d.id})`}</p>)
      ) : (
        <p>No results</p>
      )}
    </div>
