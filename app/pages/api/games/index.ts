...
const handleGames = async (req: NextApiRequest, res: NextApiResponse) => {
  const { method, body } = req

  switch (method) {
    case 'POST':
      // validate body
      const validatedData = validateRequest(body, createGamesStructure)

      // Update or create data in your database
      gameAPI.addGame(validatedData.winnerId, validatedData.loserId, validatedData.underTable)
