...
const PLAYER_AMOUNT = isNaN(Number(process.argv[2])) ? 20 : Number(process.argv[2])
const GAME_AMOUNT = isNaN(Number(process.argv[3])) ? 30 : Number(process.argv[3])
...
const createTestPlayers = async () => {
  ...
  console.log(res.map(r => `${r.id}: ${r.firstName} '${r.nickname}' ${r.lastName}`).join('\n'))
