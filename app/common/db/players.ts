import type { Player as ReturnPlayer, PlayerMeta, PlayerWithStats } from '../types'
...
const createSearchIndexForAll = async () => {
  const [players] = await Promise.all([getPlayers(), deletePlayerSearchIndexes()])
...
const getPlayerMetasByString = async (
  ...
  page: number = 0,
  pageSize: number = 20
): Promise<PlayerMeta[]> => getCachedPlayerMetas(str, page, pageSize, getPlayerMetas)
...
const playerAPI = {
  ...
  searchByKeywords: (keywords: string): Promise<PlayerMeta[]> => getPlayerMetasByString(keywords),
  getPlayerStatsById,
  getTopPlayers,
}

export { playerAPI, addPlayer, createSearchIndexForAll, getPlayerMetasByString }
