...
const getGamePlayersData = async (
  ...
  const [winner, loser, winnerGames, loserGames] = await Promise.all([
    ...
    Game.count({
      where: {
        [Op.or]: [{ winnerId }, { loserId: winnerId }],
      },
    }),
    Game.count({
      where: {
        [Op.or]: [{ winnerId: loserId }, { loserId }],
      },
    }),
  ])
...
const createGame = async (
  ...
  const [winnerChange, loserChange] = getScoreChange(winner.elo, winnerGames, loser.elo, loserGames)
  const [game, ..._] = await Promise.all([
    ...
    Player.update({ elo: winner.elo + winnerChange }, { where: { id: winner.id } }),
    Player.update({ elo: loser.elo + loserChange }, { where: { id: loser.id } }),
  ])
...
const findPrevElo = async (
  ...
  return side === 'winner' ? latestGame.winnerElo : latestGame.loserElo
}
...
export { createGame, getGames, getMutualGames }
